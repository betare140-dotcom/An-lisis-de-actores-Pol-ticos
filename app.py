...
st.write(
    "Sube tu reporte para generar el informe oficial en Word con clasificación"
    " de IA."
)
...
def cargar_archivo_seguro(file, tipo_analisis, actor_nombre):
  if tipo_analisis == "Medios Tradicionales / Portales (Prensa, TV, Radio, Portales)":
    try:
      xls = pd.ExcelFile(file)
      sheets = xls.sheet_names
      # Buscar la hoja que coincida con el actor de manera flexible
      matched_sheet = next(
          (
              s
              for s in sheets
              if actor_nombre.lower() in s.lower()
              or s.lower() in actor_nombre.lower()
          ),
          sheets[0],
      )
      ...
      raise Exception(f"No se pudo leer el archivo de medios tradicionales: {e}")
  else:
    # Redes Sociales (soporta Excel multiformato y CSV)
    try:
    ...
    raise Exception(
        "No se pudo leer el archivo de redes. Asegúrate de que sea un archivo"
        " Excel (.xlsx/.xls) o CSV válido."
    )
...
uploaded_file = st.file_uploader(
    "Sube tu archivo Excel o CSV", type=["xlsx", "xls", "csv"]
)
...
if uploaded_file and actor_nombre:
  if st.button("Generar Reporte Oficial", type="primary"):
    with st.spinner("Analizando publicaciones con Inteligencia Artificial..."):
      try:
        df = cargar_archivo_seguro(uploaded_file, tipo_analisis, actor_nombre)
      except Exception as e:
        st.error(str(e))
        st.stop()
      ...
      if df is None or len(df) == 0:
        st.error(
            "El archivo seleccionado no contiene datos o la hoja del candidato"
            " está vacía."
        )
        st.stop()
      ...
      serie_fechas_raw = obtener_columna_serie(
          df_filtrado, ["Fecha", "Publish date", "Date", "Fecha de publicación"]
      )
      ...
      def clasificar_con_ia(row):
        ...
        prompt = (
            "Eres un analista político experto. Evalúa la siguiente"
            f" publicación sobre '{actor_nombre}':\n\nCRITERIO DE"
            " CLASIFICACIÓN:\n- Responde 'NEGATIVA' ÚNICAMENTE si contiene un"
            " ataque personal directo, un insulto, una descalificación"
            f" explícita o una acusación grave de mal desempeño hacia '{actor_nombre}'.\n-"
            " Responde 'POSITIVA' si reporta respaldos de líderes del"
            " partido, propuestas, giras, logros o declaraciones del"
      ...
      def obtener_3_temas_positivos_y_negativos(df_data, actor_p):
        ...
        pos_textos = str(
            df_data[df_data["sentimiento_ia"] == "POSITIVA"][
                col_texto_busqueda
            ]
            .dropna()
            .head(30)
        ...
        neg_textos = str(
            df_data[df_data["sentimiento_ia"] == "NEGATIVA"][
                col_texto_busqueda
            ]
            .dropna()
            .head(15)
        ...
        prompt = (
            "Eres un analista de comunicación política. Analiza las noticias"
            f" sobre '{actor_p}'.\n\nNOTICIAS"
            f" POSITIVAS/INFORMATIVAS:\n{pos_textos}\n\nNOTICIAS"
            f" NEGATIVAS:\n{neg_textos}\n\nINSTRUCCIONES"
            " ESTRICTAS:\n1. NO incluyas frases estadísticas como 'Predominó la"
            " cobertura favorable...'.\n2. Redacta ESTRICTAMENTE 3 temas"
            " positivos o informativos principales numerados (1., 2., 3.).\n3."
            " Redacta ESTRICTAMENTE 3 temas negativos principales numerados"
            " (1., 2., 3.). Si no existen noticias negativas, escribe"
            " simplemente: '1. No se registraron temas negativos en el"
            " periodo analizado.'\n\nFORMATO EXACTO DE SALIDA:\n1. [Primer"
            " tema positivo]\n2. [Segundo tema positivo]\n3. [Tercer tema"
            " positivo]\n\nTEMAS NEGATIVOS:\n1. [Primer tema negativo o 'No"
            " se registraron temas negativos en el periodo analizado.']"
        )
        try:
          return model.generate_content(prompt).text.strip()
        except Exception:
          return (
              "1. Acciones de trabajo en territorio e impulso a programas"
              " sociales.\n2. Coordinación institucional con la agenda de"
              " gobierno.\n3. Presencia en medios digitales y redes"
              " sociales.\n\nTEMAS NEGATIVOS:\n1. No se registraron temas"
              " negativos en el periodo analizado."
          )
      ...
      temas_3x3 = obtener_3_temas_positivos_y_negativos(
          df_filtrado, actor_nombre
      )
        ...
        if tipo_analisis == "Redes Sociales":
          if len(pos_df) > 0:
            ...
            for _, row in pos_df.iterrows():
              ...
              link = obtener_campo(
                  row,
                  ["URL", "Link", "Enlace", "Link de Nota", "Link a Testigo"],
              )
